"""
Apollo DM Waterfall (EMaaS clone) — decision maker + verified email for
~1 AMF credit/company, retargeted for Energy GreenPrint's energy-intensive ICP.

Clone of apollo-dm-waterfall/scripts/apollo_dm_waterfall.py. ONLY the ranking
prompts and the large-org title filter differ; the waterfall mechanics are
untouched. Cloned rather than parameterised because the original feeds the LIVE
Indiana healthcare pipeline (same convention as
production-directory-leads/apollo_dm_waterfall_production.py).
...
Per row (has website, no email yet):
  1. Apollo api_search by domain (FREE, no credits) -> candidate people
     ...
     (LARGE_ORG_TITLES, mirroring the ladder) so the 100-person page contains
     relevant people.
  2. Azure OpenAI GPT-4.1 ranks up to 3 candidates (P1-P3) against Sherif
     Hani's ladder: Operations Manager/Director -> Plant/Production/
     Engineering/Maintenance/Facilities Manager -> CEO/MD/GM/Owner/COO ->
     Sustainability/ESG Manager -> nobody. A row with no one on that ladder is
     left untouched rather than enriched with a lesser title. The trades
     themselves (electricians, fitters, operators, technicians) are banned:
     they are the payroll at these companies, and they are also exactly what
     the job ads that qualified the list were advertising.
...
ADMIN_RANK_SYSTEM = """You rank employees of an energy-intensive business for a \
RESCUE pass. The main ladder already ran on this company and found nobody, so
the bans on seniority are relaxed slightly.

Pick the most senior person responsible for the SITE or its operations:
1. Site Manager / Facility Manager / Works Manager / Mill Manager
2. Operations Manager / Production Manager / Plant Manager
3. General Manager / Managing Director / Owner
4. Nobody — return an empty list.

Still NEVER pick: HR or talent of any level, sales, marketing, IT, finance
clerks or bookkeepers, shift supervisors and team leaders, or the trades
themselves (electricians, fitters, boilermakers, operators, technicians) — at
these companies the trades are the payroll, not the buyer. Never pick anyone
whose title shows a DIFFERENT employer.
...
# the page. Must mirror the RANK_SYSTEM ladder below.
LARGE_ORG_TITLES = [
    "operations manager", "operations director", "director of operations",
    "plant manager", "production manager", "engineering manager",
    "maintenance manager", "facilities manager", "site manager",
    "chief executive officer", "ceo", "managing director", "general manager",
    "owner", "founder", "president", "managing partner", "principal",
    "chief operating officer", "coo",
    "sustainability manager", "esg manager",
]

RANK_SYSTEM = """You rank employees of an energy-intensive business (food \
processing, manufacturing, cold storage, aged care, hospitality) by who most \
likely owns the decision to bring in an external energy-management service.

The offer: a free energy audit of the site, and if they proceed, monitoring
hardware and management delivered as a service, funded out of the savings with
no capital outlay. So the buyer is whoever owns the site's operating
performance and its energy cost, not a procurement or finance clerk.

DECISION LADDER (Sherif Hani, the client, is a chartered electrical engineer
with 20 years on Australian industrial sites — this is his ladder and it is
deliberate). Work strictly down this list and stop at the first person you can
find:

1. Operations Manager / Operations Director / Director of Operations.
2. Plant Manager / Production Manager / Engineering Manager /
   Maintenance Manager / Facilities Manager / Site Manager / Works Manager /
   Mill Manager. The people who run the plant and its equipment.
3. CEO / Managing Director / General Manager / Owner / Founder / President /
   Managing Partner / Principal / Proprietor / COO. The fallback when no
   operations contact exists — common at owner-operated sites, which is much
   of this list.
4. Sustainability Manager / ESG Manager — only where the role genuinely
   exists. Rare below enterprise scale in Australia.
5. Nobody. Return an EMPTY LIST.

Never invent a sixth rung. If nobody on the ladder is in the candidate list,
returning nothing is the correct answer: a lesser title spends an AnyMailFinder
credit, burns the company for any future approach, and will not reply.

NEVER pick, at any size, for any reason:
- HR of any level, people and culture, talent acquisition, internal recruiters.
- Sales, marketing, business development, account management.
- IT, software, data, or digital roles.
- Finance clerks, bookkeepers, accounts payable, payroll, procurement officers.
- Shift supervisors, team leaders, leading hands, foremen — they run a shift,
  not the site.
- The trades and operators themselves: electricians, fitters, boilermakers,
  refrigeration technicians, process operators, machine operators, drivers,
  storepersons, pickers. At these companies the trades ARE the payroll. This
  is the single most common failure mode on this list, because the job ads
  that qualified these companies were advertising exactly these roles.
- Anyone whose title shows they work for a DIFFERENT company, an agency, or a
  contractor servicing this site.

Multi-site organisations: prefer company-level or head-office titles over a
single-site role when both appear, since the service is usually rolled out
across a portfolio. A site-scoped title still counts if it is the only
operations contact available.
...
def amf_find(full_name, domain):
    """Returns (email, credits_charged) — email only when status == valid AND
    the address sits on the company's own domain.

    The domain guard exists because AMF will happily return a real, verified
    person at a DIFFERENT organisation: on the first EMaaS batch, Ryan Meat
    Company resolved to the CEO of ausmeat.com.au, the industry standards
    body. That is a wasted credit and a badly misdirected send. Same rule as
    amf_person_fill.py, which measured ~29% wrong-person hits without it."""
    try:
        resp = requests.post(
            AMF_PERSON_URL,
    ...
    if data.get("email_status") == "valid" and data.get("valid_email"):
        ...
        want, got = _domain_root(domain), _domain_root(email.split("@")[-1])
        if want and got and want != got and want not in got and got not in want:
            print(f"    [!] domain mismatch: {email} is not on {domain} — rejected")
            return None, credits
