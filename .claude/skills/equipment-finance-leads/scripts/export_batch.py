"""
Phase 3 — export a campaign batch of DEALER rows to a new Google Sheet, and
stamp them exported_at + batch_id in the store.

Deltas by default (exported_at IS NULL), so the same company is never worked
twice across batches. One row per domain (highest review count wins — the
busiest listing is usually the real HQ); domain-less rows (social-only
websites) are held back until a domain-resolution pass fills them.
...
K/L/R/S, DM block at T-W, first/last/body/pushed at X-AA, status at AB — so
apollo-dm-waterfall, exa-website-enrichment and the AMF companions run against
it with their default flags.

Usage:
  python3 -W ignore .claude/skills/equipment-finance-leads/scripts/export_batch.py [--size 500] \
      [--metros NYC,LI] [--dry_run]
"""
import argparse
import json
import os
...
def main():
    ...
    where = "classification='DEALER' AND exported_at IS NULL AND domain IS NOT NULL"
    params = []
    if args.metros:
    ...
    print(f"Pool: {len(pool)} unexported dealers "
    ...
    svc = get_service()
    title = f"Equipment Finance - NY Dealers - Batch {next_id} - {date.today().isoformat()}"
