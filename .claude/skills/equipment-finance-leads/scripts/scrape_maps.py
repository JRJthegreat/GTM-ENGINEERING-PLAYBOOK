"""
Phase 1 — scrape medical/dental equipment DEALERS from Google Maps into the
SQLite store (companies table).

Runs Apify compass/crawler-google-places once per location query (all search
terms in one run), polls to completion, upserts by place_id — re-running never
duplicates and only fills newly discovered places.

Google Maps returns name + WEBSITE + city in one pass, so we skip the Exa
domain-resolution cost the sheet pipelines pay. Rows are stored raw;
classification (Phase 2) decides what is an independent DEALER vs an actual
practice, a manufacturer, a captive-finance national, or a repair-only shop.

Usage:
  # cost/scope preview — fires nothing
  python3 -W ignore .claude/skills/equipment-finance-leads/scripts/scrape_maps.py --dry_run

  # VALIDATION scrape: two metros, small cap — prove the pool before committing
  ...
  # fallback: ingest an Apify dataset fired earlier
  python3 -W ignore .claude/skills/equipment-finance-leads/scripts/scrape_maps.py \
      --dataset_id DATASET_ID --metro_key NYC
"""
import argparse
import os
import sys
...
def start_run(terms, location_query, max_per_search):
    body = {
...
def upsert_items(conn, items, metro_key, term_label):
    """Insert new places; keep existing rows untouched (classification survives)."""
...
def main():
    ...
    if args.dry_run:
        for m in metros:
            print(f"  {m['key']:4s} {m['label']:18s} -> {m['queries']}")
    ...
    for m in metros:
        for q in m["queries"]:
            print(f"\n[{m['key']}] {q} — starting run...", flush=True)
            try:
                run_id, dataset_id = start_run(terms, q, max_per)
            ...
            print(f"  {len(items)} places -> {new} new, {dupes} already stored, {skipped} skipped", flush=True)
