"""
Stage 3 of the trades DM waterfall — AMF /decision-maker for TRADES firms via AnyMail Finder
...
def main():
    ...
    for i, row in enumerate(rows):
        ...
        # TRADES CLONE: stage 3 of Jude's waterfall runs LAST, over whatever
        # Purple Magic (pm_*) and Apollo (ap_*) could not close. Status-prefix
        # matching kept missing lanes as they were added, so the rule here is
        # simply "no email yet" — the only state that actually matters.
        has_email = (row[COL_DM_EMAIL].strip()
                     if len(row) > COL_DM_EMAIL and row[COL_DM_EMAIL] else "")
        pending = not has_email
