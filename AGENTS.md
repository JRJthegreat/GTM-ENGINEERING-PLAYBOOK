...
NEXAM AI's recruitment lead generation system — Codex skills and Python scripts that scrape job postings, find decision makers, discover emails, generate personalized outreach, and push leads to Instantly campaigns. All code lives under `.Codex/` (skills, agents, auth, env). There are no top-level source files — the root `.html` is a generated campaign report, not code.

**A Google Sheet is the database.** Almost every script reads a sheet, enriches rows in place, and writes back — there is no local model layer, no ORM, no intermediate store. That is why column constants, batch-of-10 writes, and idempotent skip-if-filled logic carry so much weight below. The exceptions are `nppes-new-clinics`, `production-house-leads`, `production-directory-leads`, and `energy-emaas-leads`, which use SQLite upstream — though all of them then build sheets and work them like the rest.

## All Pipelines

Each pipeline is a Codex skill with its own `SKILL.md` (authoritative detail) and `scripts/` directory. Exceptions: `healthcare-staffing-enrichment`, `recruitment-email-gen`, and `sba-campaigns` have no `SKILL.md` — their sections below plus script docstrings are the reference.
...
python3 -W ignore .Codex/skills/hr-leads-indeed/scripts/scrape_and_pull.py \
  --sheet_url "SHEET_URL" --limit 100 --days 14

# Example — TheirStack pipeline
python3 -W ignore .Codex/skills/scrape-hr-leads/scripts/scrape_leads.py \
...
## The Codex-in-session Judge Pattern (Aug 2026)

A growing class of scripts makes **no LLM API call at all**. Where an earlier script would have called GPT-4.1 or Codex per row, the judgment is split into three steps and the model is *this Codex session*:
...
| `civil-engineering-leads-indeed` | Codex Haiku | Codex Opus 4.5 |
| All others | Codex Haiku (via `ANTHROPIC_API_KEY`) | Codex (model per skill) |
...
- API keys: `.Codex/.env` (loaded via `dotenv` relative to script location)
...
- Google Sheets OAuth: `.Codex/token.json` (setup via `.Codex/setup_google_auth.py`)

Gitignored and therefore absent on a fresh clone: `.Codex/.env`, `.Codex/token.json` (+ `.Codex/token_*.json`), `.Codex/scripts/` (see below), `.Codex/cache/` and `.Codex/backups/` (local working state — e.g. `cache/linkedin_companies.json`, `backups/contacted_emails_*.json`), `.Codex/scheduled_tasks.lock`, `.playwright-mcp/`, and every skill `data/` directory that holds a SQLite store or working JSON — `nppes-new-clinics/data/`, `healthcare-staffing-enrichment/data/`, `production-house-leads/data/`, `production-directory-leads/data/`, `production-demand-leads/data/`, `hirebase-healthcare-leads/data/`, `energy-emaas-leads/data/`. A SQLite-backed skill therefore looks empty on a clone; the store rebuilds by re-running phase 1. `.Codex/campaign_archive` is a symlink to a private archive directory outside the repo — see Shared Utility Scripts.
...
`.Codex/scripts/` contains one-off and cross-pipeline utilities (not part of any skill's standard pipeline). **This directory is gitignored** — it exists only on Jude's machine and will be absent on a fresh clone, so never assume these scripts are present; check before referencing one. Current local contents:
- `ingest_apify.py` — generic Apify dataset ingestion
- `research_dm.py` — standalone DM research
- `archive_campaign.py` — archives a completed Instantly campaign's config, analytics, leads, and reply emails into `.Codex/campaign_archive/<campaign-id>/` (`campaign.json`, `analytics.json`, `leads.jsonl`, `emails.jsonl`). That path is a **symlink to a separate local directory outside this repo** (`~/nexam-campaign-archive`) — it contains lead PII and must never be committed here or made public
...
⚠️ **The SKILL.md is stale on the core mechanism.** It still describes a mechanical acceptance ladder; that was replaced (2026-08-10, commits `b49b107`/`882ccbb`) by the three-step Codex-in-session judge flow described above — `--apply` collects candidates, Codex judges in-session, `--verdicts FILE --apply` writes. The mechanical parts that survive are the *prefilters* (junk hosts, careers subdomains, parent-chain), not the acceptance decision. Cost/`EXA_API_KEY` details in the SKILL.md are still accurate.
...
| Icebreaker 2-3 | `collect_icebreaker_research.py` → *Codex judges* → `apply_icebreakers.py` (→AF icebreaker, AG fact_type) | Codex-in-session judge (above). No LLM API anywhere in this lane (Jude, 2026-08-12) |
...
(`collect_classification.py` → *Codex judges* → `apply_classification.py`,
plus `delete_rows.py` for approved removals — backs up every row first) →
identity recovery (`collect_identity_recovery.py` → *Codex judges* →
...
`SKILL.md` is the reference but already drifted: it lists `export_batch.py` and the Codex-in-session judge phase as "planned" — `export_batch.py` shipped (commit `49ce765`), and `scrape_indeed_signals.py` grew a `--state` flag so NSW/QLD grids share the scraper (commit `ab2f74a`). Indeed's `employeesCount` is the GLOBAL headcount band for multinationals — a hint, never a gate.
